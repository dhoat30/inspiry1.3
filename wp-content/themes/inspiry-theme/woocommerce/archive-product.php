...
if ( wc_get_loop_prop( 'total' ) ) {
    			...
    			while ( have_posts() ) {
					...
					productObject['<?php echo get_permalink( $product->get_id()); ?>'] = thisProduct;
					<?php
	...
	for(var i = 0; i < products.length; i++) {
		...
		products[i].addEventListener("click", function(event) {
            ...
            var clickedProductURL = event.currentTarget.href;
			var clickedProduct = productObject[clickedProductURL];
			console.log(clickedProduct);
  	dataLayer.push({
		    'event': 'productClickButton',
		    'ecommerce': {
		      'click': {
		        'actionField': {'list': 'yo change this value later'},      // Optional list property.
		        'products': [{
					'name': '<?php echo $product -> get_name()?>',   
       						'id': '<?php echo $product -> get_id()?>',
       						'price': '<?php echo $product -> get_price()?>',
