...
function geodir_filter_listings_where_set_loc( $term_id, $taxonomy, $post_type, $location_type, $loc, $count_type ) {
	...
	$where = '';
	if ( $country!= '') {
		$where .= " AND locations LIKE '%,[".$country."]' ";
	}

	if ( $region != '' && $location_type!='gd_country' ) {
		$where .= " AND locations LIKE '%,[".$region."],%' ";
	}

	if ( $city != '' && $location_type!='gd_country' && $location_type!='gd_region' ) {
		$where .= " AND locations LIKE '[".$city."],%' ";
	}
	
	if ($location_type == 'gd_neighbourhood' && $neighbourhood != '' && $wpdb->get_var("SHOW COLUMNS FROM " . $table . " WHERE field = 'neighbourhood'")) {
		$where .= " AND neighbourhood = '" . $neighbourhood . "' ";
	}
