...
class GeoDir_Location_Admin_Countries {
	...
	private static function table_list_output() {
		...
		echo '<h2>' . __( 'Countries', 'geodirlocation' ) . '</h2>';
		...
		if ( absint( $count ) && $count > 0 ) {
			...
			echo '<div><p>' . __( 'COUNTRY SLUG TRANSLATION: Translate the countries via .po file that you want and then upload the .mo file to your server, then tick the countries you translated and click the Apply button. This will translate country slug in url.', 'geodirlocation' ) . '</p></div>';
			echo '</div>';
		} else {
			?>
	...
	private static function form_output() {
		...
		if ( ! empty( $country ) ) {
			...
			$seo->cpt_desc = ! empty( $seo->cpt_desc ) ? json_decode( stripslashes( $seo->cpt_desc ), true ) : array();
