...
class GeoDir_Location_Admin_Neighbourhoods {
	...
	private static function table_list_output() {
		...
		echo '<h2>' . __( 'Neighbourhoods', 'geodirlocation' );
		if ( ! empty( $_REQUEST['location_id'] ) ) {
			echo ' <a href="' . esc_url( admin_url( 'admin.php?page=gd-settings&tab=locations&section=neighbourhoods&add_neighbourhood=1&location_id=' . (int)$_REQUEST['location_id'] ) ) . '" class="add-new-h2">' . __( 'Add New', 'geodirlocation' ) . '</a>';
		}
		echo '</h2>';
	...
	private static function get_data( $id ) {
		...
		$neighbourhood->cpt_desc = ! empty( $neighbourhood->cpt_desc ) ? json_decode( stripslashes( $neighbourhood->cpt_desc ), true ) : array();
