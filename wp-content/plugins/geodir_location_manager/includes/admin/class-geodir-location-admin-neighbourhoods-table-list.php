...
class GeoDir_Location_Admin_Neighbourhoods_Table_List extends WP_List_Table {
	...
	public function column_name( $item ) {
		return $item['name'] . '<small class="gd-meta">' . wp_sprintf( __( 'ID: %d', 'geodirlocation' ), $item['id'] ) . '</small>';
	}
	...
	public function prepare_items() {
		...
		$per_page = apply_filters( 'geodir_location_neighbourhoods_settings_items_per_page', 10 );
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$where[] = "h.hood_name LIKE '%" . esc_sql( $wpdb->esc_like( geodir_clean( $_REQUEST['s'] ) ) ) . "%'";
		}
