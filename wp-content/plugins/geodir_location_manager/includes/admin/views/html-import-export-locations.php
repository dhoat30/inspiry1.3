...
if ( ! empty( $results ) ) {
	foreach ( $results as $row ) {
		$count += $row->count;
		$country_options .= '<option value="' . esc_attr( $row->country ) . '">' . __( $row->country, 'geodirectory' ) . ' ' . '(' . $row->count . ')' . '</option>';
	}
}
$country_options = '<option value="">' . wp_sprintf( __( 'All (%d)', 'geodirlocation' ), $count ) . '</option>' . $country_options;
?>
<div class="inner_content_tab_main gd-import-export">
	<div class="gd-content-heading">
		...
		?>
		<div id="gd_ie_imlocations" class="metabox-holder">
			<div class="meta-box-sortables ui-sortable">
				<div id="gd_ie_im_locations" class="postbox gd-hndle-pbox">
					<button class="handlediv button-link" type="button"><span class="screen-reader-text"><?php _e( 'Toggle panel - Locations: Import CSV', 'geodirlocation' );?></span><span aria-hidden="true" class="toggle-indicator"></span></button>
					<h3 class="hndle gd-hndle-click"><span style='vertical-align:top;'><?php echo __( 'Locations: Import CSV', 'geodirlocation' );?></span></h3>
					<div class="inside">
							...
							<tr>
								<td class="gd-imex-box">
									<div class="gd-im-choices">
										<p><input type="radio" value="update" name="gd_im_choicelocation" id="gd_im_choice_u" /><label for="gd_im_choice_u"><?php _e( 'Update item if location with location_id/city_slug already exists.', 'geodirlocation' );?></label></p>
										<p><input type="radio" checked="checked" value="skip" name="gd_im_choicelocation" id="gd_im_choice_s" /><label for="gd_im_choice_s"><?php _e( 'Ignore item if location with location_id/city_slug already exists.', 'geodirlocation' );?></label></p>
									</div>
									<div class="plupload-upload-uic hide-if-no-js" id="gd_im_locationplupload-upload-ui">
										<input type="text" readonly="readonly" name="gd_im_location_file" class="gd-imex-file gd_im_location_file" id="gd_im_location" onclick="jQuery('#gd_im_locationplupload-browse-button').trigger('click');" />
										<input id="gd_im_locationplupload-browse-button" type="button" value="<?php esc_attr_e( 'Select & Upload CSV', 'geodirlocation' ); ?>" class="gd-imex-cupload button-primary" /> <input type="button" value="<?php echo esc_attr_e( 'Download Sample CSV', 'geodirlocation' );?>" class="button-secondary" name="gd_ie_download_sample" id="gd_ie_download_sample" data-sample-csv="<?php echo $sample_csv;?>">
									...
									<span id="gd_im_locationupload-error" style="display:none"></span>
									...
									<div class="gd-imex-btns" style="display:none;">
										...
										<input onclick="geodir_location_prepare_import(this, 'location')" type="button" value="<?php esc_attr_e( 'Import data now', 'geodirlocation' ); ?>" id="gd_import_data" class="button-primary" />
										<input onclick="geodir_location_resume_import(this, 'location')" type="button" value="<?php _e( "Continue Import Data", 'geodirlocation' ); ?>" id="gd_continue_data" class="button-primary" style="display:none"/>
										<input type="button" value="<?php _e("Terminate Import Data", 'geodirlocation');?>" id="gd_stop_import" class="button-primary" name="gd_stop_import" style="display:none" onclick="geodir_location_terminate_import(this, 'location')"/>
		...
		<div id="gd_ie_exlocations" class="metabox-holder">
			<div class="meta-box-sortables ui-sortable">
				<div id="gd_ie_ex_locations" class="postbox gd-hndle-pbox">
					...
					<h3 class="hndle gd-hndle-click"><span style='vertical-align:top;'><?php echo __( 'Locations: Export CSV', 'geodirlocation' );?></span></h3>
					<div class="inside">
						<table class="form-table">
							<tbody>
							<tr>
								<td class="fld"><label for="gd_country"><?php _e( 'Country:', 'geodirlocation' );?></label></td>
								<td>
									<select id="gd_country" name="gd_imex[country]" data-type="country" data-placeholder="<?php esc_attr_e( 'Filter a country...', 'geodirlocation' ); ?>" class="regular-text"><?php echo $country_options; ?></select></td>
							</tr>
							<tr>
							...
							<tr class="gd-ie-actions">
								<td style="vertical-align:top">
									<input data-export="locations" type="submit" value="<?php echo esc_attr( __( 'Export CSV', 'geodirlocation' ) );?>" class="button-primary" name="gd_start_export" id="gd_start_export">
								</td>
