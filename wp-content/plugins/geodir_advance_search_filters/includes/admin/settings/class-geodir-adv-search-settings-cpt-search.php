...
if ( ! class_exists( 'GeoDir_Adv_Search_Settings_Cpt_Search', false ) ) :
	...
	class GeoDir_Adv_Search_Settings_Cpt_Search extends GeoDir_Settings_Page {
		...
		public static function left_panel_title() {
			return sprintf( __( 'Available advance search option for %s listing and search results.', 'geodiradvancesearch' ), get_post_type_singular_label( self::$post_type, false, true ) );
		...
		public function left_panel_note() {
			return sprintf( __( 'Click on any box below to make it appear in advance search form on %s listing and search results.', 'geodiradvancesearch' ), get_post_type_singular_label( self::$post_type, false, true ) );
		}
		...
		public static function right_panel_title() {
			return sprintf( __( 'List of fields those will appear in advance search form on %s listing and search results page.', 'geodiradvancesearch' ), get_post_type_singular_label( self::$post_type, false, true ) );
		}
		...
		public function right_panel_note() {
			return sprintf( __( 'Click to expand and view field related settings. You may drag and drop to arrange fields order in advance search form on %s listing and search results page.', 'geodiradvancesearch' ), get_post_type_singular_label( self::$post_type, false, true ) );
		}
					...
					<div class="field_row_main">
						<div class="dd gd-tabs-layout" >
							<ul class="dd-list gd-tabs-sortable gd-sortable-sortable">
								...
								$fields = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . GEODIR_ADVANCE_SEARCH_TABLE . " WHERE post_type = %s AND field_type != 'address' ORDER BY sort_order ASC", array( self::$post_type ) ) );

								if ( ! empty( $fields ) ) {
									echo self::loop_fields_output($fields);
								} else {
									_e( 'Select fields from the left to be able to add new search options.', 'geodiradvancesearch' );
								}
								?>
							</ul>
						</div>
			...
			if ( ! empty( $fields ) ) {
				...
				foreach ( $fields as $key => $field ) {
					...
					if ( isset( $field['field_icon'] ) && geodir_is_fa_icon( $field['field_icon'] ) ) {
						$field_icon = '<i class="' . sanitize_text_field($field['field_icon']) . '" aria-hidden="true"></i>';
					} elseif ( isset( $field['field_icon'] ) && geodir_is_icon_url( $field['field_icon'] ) ) {
						$field_icon = '<b style="background-image: url("' . sanitize_text_field($field['field_icon']) . '")"></b>';
					} else {
						$field_icon = '<i class="fas fa-cog" aria-hidden="true"></i>';
					}
					?>
					<li id="geodir-field-item-<?php echo $field['htmlvar_name']; ?>" <?php echo $display; ?>>
						<a id="gd-<?php echo $field['htmlvar_name']; ?>"
						   class="gd-draggable-form-items gd-fieldset" 
						   href="javascript:void(0);" 
						   data-htmlvar_name="<?php echo $field['htmlvar_name']; ?>" 
